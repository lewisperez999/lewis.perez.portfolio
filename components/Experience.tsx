...
export function Experience() {
  ...
  <ol className="relative border-s border-black dark:border-white/10 hover:border-blue-500/50 transition-colors duration-500">
          {experience.map((job, idx) => (
            <li key={job.role + job.company} className="group ms-6 pb-10 last:pb-0 animate-fade-in-up hover:translate-x-2 transition-all duration-300" style={{animationDelay: `${idx * 150}ms`}}>
              <span className="absolute -start-3 mt-1.5 flex h-6 w-6 items-center justify-center rounded-full bg-blue-600 text-white text-[10px] font-semibold group-hover:bg-blue-500 group-hover:scale-110 transition-all duration-300">
                {idx + 1}
              </span>
              ...
              <ul className="list-disc ms-5 space-y-1 text-sm text-foreground/80 group-hover:text-foreground transition-colors duration-300">
                {job.points.map(p => (
                  <li key={p} className="hover:text-blue-600 transition-colors duration-200">{p}</li>
                ))}
              </ul>
            </li>
          ))}
